...
export class LectureController {
  ...
  async create(
    ...
    try {
      ...
      res
        .status(Error.CREATE_LECTURE.status)
        .send(Error.CREATE_LECTURE.description);
    }
  }
  ...
  async findAll(@Res() res: Response) {
    try {
      ...
      this.logger.debug(error);
      res
        .status(Error.GET_LECTURES.status)
        .send(Error.GET_LECTURES.description);
    }
  }
  ...
  async update(
    ...
    try {
      ...
      this.logger.debug(error);
      res
        .status(Error.UPDATE_LECTURE.status)
        .send(Error.UPDATE_LECTURE.description);
    }
  }
  ...
  async remove(@Res() res: Response, @Param('id') id: string): Promise<void> {
    try {
      ...
      this.logger.debug(error);
      res
        .status(Error.DELETE_LECTURE.status)
        .send(Error.DELETE_LECTURE.description);
    }
  }
}
